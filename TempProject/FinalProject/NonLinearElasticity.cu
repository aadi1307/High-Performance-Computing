...
void NLElasticity::createHexGridMeshGeneral(int nElemX, int nElemY, int nElemZ, double sizeX, double sizeY, double sizeZ)
{
   ...
   // std::cout << "myNumDOFCreateHex: " << myNumDOF << std::endl;
...
void NLElasticity::applyDirichletOnSurfaceFunction(const std::string& uString,
    ...
    // std::cout << "myFixedNodes: ";
    // for (const int& node : myFixedNodes) {
    //     std::cout << node << " ";
    // }
    // std::cout << std::endl;
}
...
void NLElasticity::applyNeumanOnSurface(double forceU, double forceV, double forceW, size_t SurfaceNum) {
    ...
    // Print myBCtype
    // std::cout << "myBCtype:" << std::endl;
    // for (const auto& row : myBCtype) {
    ...
    // Print myBCvalue
    // std::cout << "myBCvalue:" << std::endl;
    // for (const auto& row : myBCvalue) {
...
void NLElasticity::assembleBC() {
   ...
   // Assuming xi_GQ.size() is the correct size
    ...
    for (size_t i = 0; i < xi_GQ[0].size(); ++i) {
       ...
       // std::cout << "N2DEigenTransposed " << N2DEigenTransposed << std::endl;
         // Print N2DCell
        // std::cout << "N2DCell:" << std::endl;
        // for (const auto& matrix : N2DCell) {
        //     std::cout << matrix << std::endl;
        //     std::cout << std::endl;
        // }

        Eigen::Map<Eigen::MatrixXd> gradNEigen(gradN2D.data()->data(), gradN2D[0].size(), gradN2D.size());
        Eigen::MatrixXd gradNEigenTransposed = gradNEigen.transpose();
        gradN2DCell.push_back(gradNEigenTransposed);
    ...
    for (int geomEdge = 0; geomEdge <= numEdges; ++geomEdge) {
       ...
       // std::cout << "valuew " << valuew << "geomEdge " << geomEdge <<std::endl;
        if (valuew == -8000)
        {
            int temp = 1;
        }
...
void NLElasticity::solveNonlinearFEM() {
    ...
    for (int step = 1; step <= nSteps; ++step) {
        ...
        while (iter < maxIter && err > tol) {
          ...
          //  std::cout << "dSolSp " << dSolSp << std::endl;

            // Solving:
            //dSol = myK.inverse() * (-b);
            //std::cout << "dSol " << dSol << std::endl;
            
            myDeltaSol = dSolSp.head(myNumDOF);
            mySol += myDeltaSol;

            //td::cout << "mySol " << mySol << std::endl;
